...
onMounted(async () => {
  ...
  if (newsStore.newsDetail.id) {
    ...
    if (userStore.getLoginStatus) {
      try {
        const result = await historyStore.addHistory(newsStore.newsDetail.id);
  ...
  if (userStore.getLoginStatus && newsStore.newsDetail.id) {
    const result = await favoriteStore.checkFavoriteStatus(newsStore.newsDetail.id)
    if (result.success && !result.isLocal) {
      // 如果API请求成功且不是本地状态，同步服务器状态到本地
      if (result.isFavorite && !favoriteStore.isFavorite(newsStore.newsDetail.id)) {
        // 服务器已收藏但本地未收藏，添加到本地列表
        favoriteStore.favorites.unshift({
        ...
        // 服务器未收藏但本地已收藏，从本地列表移除
        favoriteStore.favorites = favoriteStore.favorites.filter(item => item.id !== newsStore.newsDetail.id)
      }
    }
  }
})
